# PCB_MathPolygon.calculateBBoxWidth
...
当你需要理解 PCB_MathPolygon.calculateBBoxWidth 的调用方式、返回值和使用边界时，这个案例可以作为起点。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_MathPolygon_calculateBBoxWidth_' + Date.now();
  ...
  const result = await eda.pcb_MathPolygon.calculateBBoxWidth(undefined);
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_MathPolygon.calculateBBoxWidth`。
