# PCB_PrimitiveComponent.getAllPinsByPrimitiveId
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitiveComponent_getAllPinsByPrimitiveId_' + Date.now();
  ...
  try {
    const result = await eda.pcb_PrimitiveComponent.getAllPinsByPrimitiveId(primitiveId);
    return {
      fixtureName,
...
- 返回值：Promise<Array<[IPCB\_PrimitiveComponentPad](./IPCB_PrimitiveComponentPad.md) > \| undefined>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveComponent.getAllPinsByPrimitiveId`。
