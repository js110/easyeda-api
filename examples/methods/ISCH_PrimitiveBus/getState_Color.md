# ISCH_PrimitiveBus.getState_Color
...
return await (async function(eda) {
  const fixtureName = '__codex_example___ISCH_PrimitiveBus_getState_Color_' + Date.now();
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `ISCH_PrimitiveBus.getState_Color`。
