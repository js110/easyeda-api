...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitiveWire_modify_' + Date.now();
  ...
  try {
    const before = await eda.sch_PrimitiveWire.get(primitiveId);
    const result = await eda.sch_PrimitiveWire.modify(primitiveId, property);
    const after = await eda.sch_PrimitiveWire.get(primitiveId);
    return {
      fixtureName,
      primitiveId,
      property,
...
- 返回值：Promise<[ISCH\_PrimitiveWire](./ISCH_PrimitiveWire.md) \| undefined>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitiveWire.modify`。
