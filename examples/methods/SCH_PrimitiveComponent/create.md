...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitiveComponent_create_' + Date.now();
...
- `x`：坐标 X
- `y`：坐标 Y
- `subPartName`：_（可选）_ 子图块名称
- `rotation`：_（可选）_ 旋转角度
- `mirror`：_（可选）_ 是否镜像
- `addIntoBom`：_（可选）_ 是否加入 BOM
- `addIntoPcb`：_（可选）_ 是否转到 PCB
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitiveComponent.create`。
