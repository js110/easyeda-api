...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitiveFill_delete_' + Date.now();
  ...
  const deleted = await eda.pcb_PrimitiveFill.delete([primitiveId]);
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveFill.delete`。
