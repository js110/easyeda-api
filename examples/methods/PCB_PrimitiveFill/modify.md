...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitiveFill_modify_' + Date.now();
  ...
  const createdPrimitive = await eda.pcb_PrimitiveFill.create(
    1,
    fixturePolygon,
    'GND',
    undefined,
  ...
  try {
    const before = await eda.pcb_PrimitiveFill.get(primitiveId);
    const result = await eda.pcb_PrimitiveFill.modify(primitiveId, property);
    const after = await eda.pcb_PrimitiveFill.get(primitiveId);
    return {
      fixtureName,
      primitiveId,
      property,
    ...
    if (primitiveId) { await eda.pcb_PrimitiveFill.delete([primitiveId]); }
  }
})(eda);
```
...
- 返回值：Promise<[IPCB\_PrimitiveFill](./IPCB_PrimitiveFill.md) \| undefined>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveFill.modify`。
