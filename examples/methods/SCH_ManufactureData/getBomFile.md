...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_ManufactureData_getBomFile_' + Date.now();
  ...
  const result = await eda.sch_ManufactureData.getBomFile(undefined, undefined, undefined, undefined, undefined, undefined, [], undefined);
  return {
    fixtureName,
...
- `assemblyVariantsConfig`：_（可选）_ 装配体变量配置
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_ManufactureData.getBomFile`。
