...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitiveLine_modify_' + Date.now();
  ...
  const createdPrimitive = await eda.pcb_PrimitiveLine.create(
    'GND',
    1,
    1000,
    1000,
  ...
  try {
    const before = await eda.pcb_PrimitiveLine.get(primitiveId);
    const result = await eda.pcb_PrimitiveLine.modify(primitiveId, property);
    const after = await eda.pcb_PrimitiveLine.get(primitiveId);
    return {
      fixtureName,
      primitiveId,
      property,
    ...
    if (primitiveId) { await eda.pcb_PrimitiveLine.delete([primitiveId]); }
  }
})(eda);
```
...
- 返回值：Promise<[IPCB\_PrimitiveLine](./IPCB_PrimitiveLine.md) \| undefined>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveLine.modify`。
