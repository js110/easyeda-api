...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitivePoured_delete_' + Date.now();
  ...
  const result = await eda.pcb_PrimitivePoured.delete(primitiveId);
...
- `primitiveIds`：覆铜填充的图元 ID 或覆铜填充图元对象
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitivePoured.delete`。
