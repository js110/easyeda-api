# ISCH_PrimitiveArc.getState_LineType
...
return await (async function(eda) {
  const fixtureName = '__codex_example___ISCH_PrimitiveArc_getState_LineType_' + Date.now();
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `ISCH_PrimitiveArc.getState_LineType`。
