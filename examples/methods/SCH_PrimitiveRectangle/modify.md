...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitiveRectangle_modify_' + Date.now();
  ...
  const createdPrimitive = await eda.sch_PrimitiveRectangle.create(
    120,
    80,
    60,
    40,
  ...
  try {
    const before = await eda.sch_PrimitiveRectangle.get(primitiveId);
    const result = await eda.sch_PrimitiveRectangle.modify(primitiveId, property);
    const after = await eda.sch_PrimitiveRectangle.get(primitiveId);
    return {
      fixtureName,
      primitiveId,
      property,
    ...
    if (primitiveId) { await eda.sch_PrimitiveRectangle.delete([primitiveId]); }
  }
})(eda);
```
...
- 返回值：Promise<[ISCH\_PrimitiveRectangle](./ISCH_PrimitiveRectangle.md) \| undefined>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitiveRectangle.modify`。
