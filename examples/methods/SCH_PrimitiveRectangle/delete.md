...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitiveRectangle_delete_' + Date.now();
  ...
  const deleted = await eda.sch_PrimitiveRectangle.delete([primitiveId]);
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitiveRectangle.delete`。
