...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitiveRectangle_create_' + Date.now();
...
- `width`：宽
- `height`：高
- `cornerRadius`：_（可选）_ 圆角半径
- `rotation`：_（可选）_ 旋转角度，绕左上点旋转，可选 `0` `90` `180` `270`
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitiveRectangle.create`。
