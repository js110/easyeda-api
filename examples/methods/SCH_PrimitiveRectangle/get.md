...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitiveRectangle_get_' + Date.now();
  ...
  try {
    const result = await eda.sch_PrimitiveRectangle.get(primitiveId);
    return {
      fixtureName,
...
- `primitiveIds`：矩形的图元 ID，可以为字符串或字符串数组，如若为数组，则返回的也是数组
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitiveRectangle.get`。
