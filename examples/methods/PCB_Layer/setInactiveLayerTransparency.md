# PCB_Layer.setInactiveLayerTransparency
...
当你准备把 setInactiveLayerTransparency 用在批量改图、重命名或清理脚本里时，先用这个案例确认前置条件和失败信号会更稳。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_Layer_setInactiveLayerTransparency_' + Date.now();
  ...
  const result = await eda.pcb_Layer.setInactiveLayerTransparency(0);
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_Layer.setInactiveLayerTransparency`。
