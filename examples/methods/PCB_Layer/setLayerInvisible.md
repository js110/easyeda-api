...
当你准备把 setLayerInvisible 用在批量改图、重命名或清理脚本里时，先用这个案例确认前置条件和失败信号会更稳。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_Layer_setLayerInvisible_' + Date.now();
  ...
  const result = await eda.pcb_Layer.setLayerInvisible(1, undefined);
...
- `layer`：_（可选）_ 层，如若不指定任何层则默认为所有层
- `setOtherLayerVisible`：_（可选）_ 是否将其它层设置为可见
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_Layer.setLayerInvisible`。
