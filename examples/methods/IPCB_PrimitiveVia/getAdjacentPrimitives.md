# IPCB_PrimitiveVia.getAdjacentPrimitives
...
return await (async function(eda) {
  const fixtureName = '__codex_example___IPCB_PrimitiveVia_getAdjacentPrimitives_' + Date.now();
...
- 返回值：Promise<Array<[IPCB\_PrimitiveLine](./IPCB_PrimitiveLine.md) \| [IPCB\_PrimitiveArc](./IPCB_PrimitiveArc.md) >>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_PrimitiveVia.getAdjacentPrimitives`。
