...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitiveImage_get_' + Date.now();
  ...
  const primitiveIdCandidates = await eda.pcb_PrimitiveImage.getAllPrimitiveId();
  ...
  const result = await eda.pcb_PrimitiveImage.get(primitiveId);
  return {
    fixtureName,
...
- `primitiveIds`：图像的图元 ID，可以为字符串或字符串数组，如若为数组，则返回的也是数组

- 返回值：Promise<[IPCB\_PrimitiveImage](./IPCB_PrimitiveImage.md) \| undefined>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveImage.get`。
