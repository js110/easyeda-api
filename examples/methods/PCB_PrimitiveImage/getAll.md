...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitiveImage_getAll_' + Date.now();
  ...
  const result = await eda.pcb_PrimitiveImage.getAll(1, false);
  return {
    fixtureName,
...
- 返回值：Promise<Array<[IPCB\_PrimitiveImage](./IPCB_PrimitiveImage.md) >>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveImage.getAll`。
