# SCH_PrimitiveAttribute.getAllPrimitiveId
...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitiveAttribute_getAllPrimitiveId_' + Date.now();
  ...
  const result = await eda.sch_PrimitiveAttribute.getAllPrimitiveId(undefined);
  return {
    fixtureName,
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitiveAttribute.getAllPrimitiveId`。
