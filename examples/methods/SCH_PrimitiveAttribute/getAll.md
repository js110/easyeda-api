...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitiveAttribute_getAll_' + Date.now();
  ...
  const result = await eda.sch_PrimitiveAttribute.getAll(undefined);
  return {
    fixtureName,
...
- `parentPrimitiveId`：_（可选）_ 父图元 ID

- 返回值：Promise<Array<[ISCH\_PrimitiveAttribute](./ISCH_PrimitiveAttribute.md) >>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitiveAttribute.getAll`。
