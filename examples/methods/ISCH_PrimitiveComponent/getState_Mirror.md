# ISCH_PrimitiveComponent.getState_Mirror
...
当你需要在写扩展前先摸清当前上下文，或者需要把 getState_Mirror 的返回值组织成后续逻辑时，这个案例最有参考价值。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___ISCH_PrimitiveComponent_getState_Mirror_' + Date.now();
  ...
  const result = await primitive.getState_Mirror();
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `ISCH_PrimitiveComponent.getState_Mirror`。
