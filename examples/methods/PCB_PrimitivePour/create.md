...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitivePour_create_' + Date.now();
...
- `pourFillMethod`：_（可选）_ 覆铜填充方法
- `preserveSilos`：_（可选）_ 是否保留孤岛
- `pourName`：_（可选）_ 覆铜名称
- `pourPriority`：_（可选）_ 覆铜优先级
- `lineWidth`：_（可选）_ 线宽
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitivePour.create`。
