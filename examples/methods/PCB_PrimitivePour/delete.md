...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitivePour_delete_' + Date.now();
  ...
  const deleted = await eda.pcb_PrimitivePour.delete([primitiveId]);
...
- `primitiveIds`：覆铜边框的图元 ID 或覆铜边框图元对象
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitivePour.delete`。
