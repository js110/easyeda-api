...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitiveText_create_' + Date.now();
...
- `textColor`：_（可选）_ 文本颜色，`null` 表示默认
- `fontName`：_（可选）_ 字体名称，`null` 表示默认
- `fontSize`：_（可选）_ 字体大小，`null` 表示默认
- `bold`：_（可选）_ 是否加粗
- `italic`：_（可选）_ 是否斜体
- `underLine`：_（可选）_ 是否加下划线
- `alignMode`：_（可选）_ 对齐模式，`0` 左顶，`1` 中顶，`2` 右顶，`3` 左中，`4` 中中，`5` 右中，`6` 左底，`7` 中底，`8` 右底

- 返回值：Promise<[ISCH\_PrimitiveText](./ISCH_PrimitiveText.md) \| undefined>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitiveText.create`。
