# SCH_PrimitiveCircle.getAllPrimitiveId
...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitiveCircle_getAllPrimitiveId_' + Date.now();
  ...
  const result = await eda.sch_PrimitiveCircle.getAllPrimitiveId();
  return {
    fixtureName,
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitiveCircle.getAllPrimitiveId`。
