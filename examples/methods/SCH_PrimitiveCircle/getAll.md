...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitiveCircle_getAll_' + Date.now();
  ...
  const result = await eda.sch_PrimitiveCircle.getAll();
  return {
    fixtureName,
...
- 返回值：Promise<Array<[ISCH\_PrimitiveCircle](./ISCH_PrimitiveCircle.md) >>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitiveCircle.getAll`。
