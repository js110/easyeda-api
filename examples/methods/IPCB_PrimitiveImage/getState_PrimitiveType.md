# IPCB_PrimitiveImage.getState_PrimitiveType
...
return await (async function(eda) {
  const fixtureName = '__codex_example___IPCB_PrimitiveImage_getState_PrimitiveType_' + Date.now();
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_PrimitiveImage.getState_PrimitiveType`。
