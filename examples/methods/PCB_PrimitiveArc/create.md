...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitiveArc_create_' + Date.now();
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveArc.create`。
