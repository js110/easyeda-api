...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitiveArc_getAll_' + Date.now();
  ...
  const result = await eda.pcb_PrimitiveArc.getAll('GND', 1, false);
  return {
    fixtureName,
...
- 返回值：Promise<Array<[IPCB\_PrimitiveArc](./IPCB_PrimitiveArc.md) >>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveArc.getAll`。
