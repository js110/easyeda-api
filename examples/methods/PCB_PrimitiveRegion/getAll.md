...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitiveRegion_getAll_' + Date.now();
  ...
  const result = await eda.pcb_PrimitiveRegion.getAll(1, [], false);
  return {
    fixtureName,
...
- `layer`：_（可选）_ 层
- `ruleType`：_（可选）_ 区域规则类型，只会匹配所有规则类型均一致的图元
- `primitiveLock`：_（可选）_ 是否锁定

- 返回值：Promise<Array<[IPCB\_PrimitiveRegion](./IPCB_PrimitiveRegion.md) >>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveRegion.getAll`。
