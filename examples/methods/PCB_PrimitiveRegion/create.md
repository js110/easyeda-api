...
- `ruleType`：_（可选）_ 区域规则类型
- `regionName`：_（可选）_ 区域名称
- `lineWidth`：_（可选）_ 线宽
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveRegion.create`。
