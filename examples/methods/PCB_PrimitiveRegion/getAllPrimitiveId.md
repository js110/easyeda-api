# PCB_PrimitiveRegion.getAllPrimitiveId
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitiveRegion_getAllPrimitiveId_' + Date.now();
  ...
  const result = await eda.pcb_PrimitiveRegion.getAllPrimitiveId(1, [], false);
  return {
    fixtureName,
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveRegion.getAllPrimitiveId`。
