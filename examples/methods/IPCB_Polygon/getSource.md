...
return await (async function(eda) {
  const fixtureName = '__codex_example___IPCB_Polygon_getSource_' + Date.now();
  ...
  if (!primitiveId) {
    throw new Error('请先在PCB画布上选中一个与 IPCB_Polygon 匹配的图元，再运行这个案例。');
  }
  const fetched = await eda.pcb_Polygon.get([primitiveId]);
...
- 返回值：[TPCB\_PolygonSourceArray](../types/TPCB_PolygonSourceArray.md)
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_Polygon.getSource`。
