...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitiveVia_create_' + Date.now();
...
- `diameter`：外径
- `viaType`：_（可选）_ 过孔类型
- `designRuleBlindViaName`：_（可选）_ 盲埋孔设计规则项名称，定义过孔的开始层与结束层，`null` 表示非盲埋孔
- `solderMaskExpansion`：_（可选）_ 阻焊/助焊扩展，`null` 表示跟随规则
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveVia.create`。
