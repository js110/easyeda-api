...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitiveVia_getAll_' + Date.now();
  ...
  const result = await eda.pcb_PrimitiveVia.getAll('GND', false);
  return {
    fixtureName,
...
- 返回值：Promise<Array<[IPCB\_PrimitiveVia](./IPCB_PrimitiveVia.md) >>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveVia.getAll`。
