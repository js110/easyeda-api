...
当你需要在写扩展前先摸清当前上下文，或者需要把 getPrimitivesInRegion 的返回值组织成后续逻辑时，这个案例最有参考价值。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_Document_getPrimitivesInRegion_' + Date.now();
  ...
  const result = await eda.pcb_Document.getPrimitivesInRegion(0, 0, 0, 0, undefined);
  return {
    fixtureName,
...
- `top`：矩形框第一 Y 坐标
- `bottom`：矩形框第二 Y 坐标
- `leftToRight`：_（可选）_ 是否仅获取完全框选的图元，`false` 则触碰即获取
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_Document.getPrimitivesInRegion`。
