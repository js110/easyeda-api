# PCB_Document.importAutoRouteSesFile
...
当你需要理解 PCB_Document.importAutoRouteSesFile 的调用方式、返回值和使用边界时，这个案例可以作为起点。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_Document_importAutoRouteSesFile_' + Date.now();
  ...
  const result = await eda.pcb_Document.importAutoRouteSesFile(undefined);
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_Document.importAutoRouteSesFile`。
