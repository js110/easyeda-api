...
当你准备把 setCanvasOrigin 用在批量改图、重命名或清理脚本里时，先用这个案例确认前置条件和失败信号会更稳。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_Document_setCanvasOrigin_' + Date.now();
  ...
  const result = await eda.pcb_Document.setCanvasOrigin(0, 0);
...
- `offsetX`：画布原点相对于数据原点的 X 坐标偏移
- `offsetY`：画布原点相对于数据原点的 Y 坐标偏移
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_Document.setCanvasOrigin`。
