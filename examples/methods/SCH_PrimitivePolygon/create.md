...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitivePolygon_create_' + Date.now();
...
- `line`：坐标组，连续的一组 `[x1, y1, x2, y2, x3, y3]` 所描述的线
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitivePolygon.create`。
