...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitivePolygon_getAll_' + Date.now();
...
- 返回值：Promise<Array<[ISCH\_PrimitivePolygon](./ISCH_PrimitivePolygon.md) >>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitivePolygon.getAll`。
