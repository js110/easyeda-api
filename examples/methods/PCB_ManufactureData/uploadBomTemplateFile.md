# PCB_ManufactureData.uploadBomTemplateFile
...
当你需要理解 PCB_ManufactureData.uploadBomTemplateFile 的调用方式、返回值和使用边界时，这个案例可以作为起点。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_ManufactureData_uploadBomTemplateFile_' + Date.now();
  ...
  const result = await eda.pcb_ManufactureData.uploadBomTemplateFile(undefined, undefined);
...
- `templateFile`：BOM 模板文件
- `template`：_（可选）_ BOM 模板名称，如若为 `undefined` 则自动从 `templateFile` 中取值
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_ManufactureData.uploadBomTemplateFile`。
