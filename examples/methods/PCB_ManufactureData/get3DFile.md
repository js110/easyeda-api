...
当你需要在写扩展前先摸清当前上下文，或者需要把 get3DFile 的返回值组织成后续逻辑时，这个案例最有参考价值。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_ManufactureData_get3DFile_' + Date.now();
  ...
  const result = await eda.pcb_ManufactureData.get3DFile(undefined, undefined, [], undefined, undefined);
  return {
    fixtureName,
...
- `element`：_（可选）_ 导出对象
- `modelMode`：_（可选）_ 导出模式，`Outfit` = 装配体，`Parts` = 零件
- `autoGenerateModels`：_（可选）_ 是否为未绑定 3D 模型的元件自动生成 3D 模型（根据元件的“高度”属性）
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_ManufactureData.get3DFile`。
