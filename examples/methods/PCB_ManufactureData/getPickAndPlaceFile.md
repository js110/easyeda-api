...
当你需要在写扩展前先摸清当前上下文，或者需要把 getPickAndPlaceFile 的返回值组织成后续逻辑时，这个案例最有参考价值。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_ManufactureData_getPickAndPlaceFile_' + Date.now();
  ...
  const result = await eda.pcb_ManufactureData.getPickAndPlaceFile(undefined, undefined, undefined);
  return {
    fixtureName,
...
- 这个参数是枚举类型 ESYS_Unit.MILLIMETER | ESYS_Unit.MIL，生成器先给出最保守的占位值，使用前建议结合对应枚举文档细化。

## 验证记录入口

默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_ManufactureData.getPickAndPlaceFile`。
