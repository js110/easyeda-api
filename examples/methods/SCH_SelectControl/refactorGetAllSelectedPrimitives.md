# SCH_SelectControl.refactorGetAllSelectedPrimitives
...
当你需要理解 SCH_SelectControl.refactorGetAllSelectedPrimitives 的调用方式、返回值和使用边界时，这个案例可以作为起点。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_SelectControl_refactorGetAllSelectedPrimitives_' + Date.now();
  ...
  const result = await eda.sch_SelectControl.refactorGetAllSelectedPrimitives();
...
- 返回值：Promise<Array<[ISCH\_Primitive](../interfaces/ISCH_Primitive.md) >>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_SelectControl.refactorGetAllSelectedPrimitives`。
