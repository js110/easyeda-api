# SCH_SelectControl.getSelectedPrimitives
...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_SelectControl_getSelectedPrimitives_' + Date.now();
  ...
  const result = await eda.sch_SelectControl.getSelectedPrimitives();
  return {
    fixtureName,
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_SelectControl.getSelectedPrimitives`。
