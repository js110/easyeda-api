# IPCB_PrimitiveDimension.getState_CoordinateSet
...
当你需要在写扩展前先摸清当前上下文，或者需要把 getState_CoordinateSet 的返回值组织成后续逻辑时，这个案例最有参考价值。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___IPCB_PrimitiveDimension_getState_CoordinateSet_' + Date.now();
  ...
  const result = await primitive.getState_CoordinateSet();
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_PrimitiveDimension.getState_CoordinateSet`。
