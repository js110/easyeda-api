...
return await (async function(eda) {
  const fixtureName = '__codex_example___LIB_Footprint_search_' + Date.now();
  ...
  const result = await eda.lib_Footprint.search('key', footprintLibraryUuid, undefined, undefined, undefined);
  return {
    fixtureName,
    result,
  };
...
- 返回值：Promise<Array<[ILIB\_FootprintSearchItem](../interfaces/ILIB_FootprintSearchItem.md) >>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `LIB_Footprint.search`。
