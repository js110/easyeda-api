...
return await (async function(eda) {
  const fixtureName = '__codex_example___LIB_Footprint_copy_' + Date.now();
  ...
  const created = await eda.lib_Footprint.copy(footprintUuid, footprintLibraryUuid, 'targetLibraryUuid', undefined, undefined);
...
- `newFootprintName`：_（可选）_ 新封装名称，如若目标库内存在重名封装将导致复制失败
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `LIB_Footprint.copy`。
