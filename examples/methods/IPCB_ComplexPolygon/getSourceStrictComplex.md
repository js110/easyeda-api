# IPCB_ComplexPolygon.getSourceStrictComplex
...
当你需要在写扩展前先摸清当前上下文，或者需要把 getSourceStrictComplex 的返回值组织成后续逻辑时，这个案例最有参考价值。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___IPCB_ComplexPolygon_getSourceStrictComplex_' + Date.now();
  ...
  if (!primitiveId) {
    throw new Error('请先在PCB画布上选中一个与 IPCB_ComplexPolygon 匹配的图元，再运行这个案例。');
  }
  const fetched = await eda.pcb_ComplexPolygon.get([primitiveId]);
  const primitive = Array.isArray(fetched) ? fetched[0] : fetched;
  if (!primitive) { throw new Error('没有取到目标图元对象。'); }
  const result = await primitive.getSourceStrictComplex();
...
- 返回值：Array<[TPCB\_PolygonSourceArray](../types/TPCB_PolygonSourceArray.md) >
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_ComplexPolygon.getSourceStrictComplex`。
