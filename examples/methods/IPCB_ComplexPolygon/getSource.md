...
当你需要在写扩展前先摸清当前上下文，或者需要把 getSource 的返回值组织成后续逻辑时，这个案例最有参考价值。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___IPCB_ComplexPolygon_getSource_' + Date.now();
  ...
  const result = await primitive.getSource();
...
- 返回值：[TPCB\_PolygonSourceArray](../types/TPCB_PolygonSourceArray.md) \| Array<[TPCB\_PolygonSourceArray](../types/TPCB_PolygonSourceArray.md) >
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_ComplexPolygon.getSource`。
