...
当你需要在写扩展前先摸清当前上下文，或者需要把 getPrimitivesBBox 的返回值组织成后续逻辑时，这个案例最有参考价值。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_Primitive_getPrimitivesBBox_' + Date.now();
  ...
  const createdPrimitive = await eda.sch_PrimitiveWire.create(
    [120, 80, 200, 80],
  );
  ...
  try {
    const result = await eda.sch_Primitive.getPrimitivesBBox([primitiveId]);
    return {
      fixtureName,
    ...
    if (primitiveId) { await eda.sch_PrimitiveWire.delete([primitiveId]); }
  }
})(eda);
```
...
- `primitiveIds`：图元 ID 数组或图元对象数组

- 返回值：Promise<{ minX: number; minY: number; maxX: number; maxY: number; } \| undefined>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_Primitive.getPrimitivesBBox`。
