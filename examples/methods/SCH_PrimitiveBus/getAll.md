...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitiveBus_getAll_' + Date.now();
  ...
  const result = await eda.sch_PrimitiveBus.getAll();
  return {
    fixtureName,
...
- 返回值：Promise<Array<[ISCH\_PrimitiveBus](./ISCH_PrimitiveBus.md) >>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitiveBus.getAll`。
