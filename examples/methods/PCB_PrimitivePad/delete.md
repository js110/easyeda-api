...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitivePad_delete_' + Date.now();
  ...
  const primitiveIdCandidates = await eda.pcb_PrimitivePad.getAllPrimitiveId();
  ...
  const result = await eda.pcb_PrimitivePad.delete(primitiveId);
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitivePad.delete`。
