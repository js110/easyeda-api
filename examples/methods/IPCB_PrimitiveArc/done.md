...
当你需要理解 IPCB_PrimitiveArc.done 的调用方式、返回值和使用边界时，这个案例可以作为起点。
...
- 返回值：Promise<[IPCB\_PrimitiveArc](./IPCB_PrimitiveArc.md) >
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_PrimitiveArc.done`。
