# IPCB_PrimitiveObject.getState_PrimitiveLock
...
return await (async function(eda) {
  const fixtureName = '__codex_example___IPCB_PrimitiveObject_getState_PrimitiveLock_' + Date.now();
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_PrimitiveObject.getState_PrimitiveLock`。
