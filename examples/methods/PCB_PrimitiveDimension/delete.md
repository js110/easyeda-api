...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitiveDimension_delete_' + Date.now();
  ...
  const deleted = await eda.pcb_PrimitiveDimension.delete([primitiveId]);
...
- `primitiveIds`：尺寸标注的图元 ID 或尺寸标注图元对象
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveDimension.delete`。
