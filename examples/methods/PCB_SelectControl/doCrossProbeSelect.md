# PCB_SelectControl.doCrossProbeSelect
...
当你需要理解 PCB_SelectControl.doCrossProbeSelect 的调用方式、返回值和使用边界时，这个案例可以作为起点。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_SelectControl_doCrossProbeSelect_' + Date.now();
  ...
  const result = await eda.pcb_SelectControl.doCrossProbeSelect(undefined, undefined, undefined, undefined, undefined);
...
- `components`：_（可选）_ 器件位号
- `pins`：_（可选）_ 器件位号\_引脚编号，格式为 \['U1\_1', 'U1\_2'\]
- `nets`：_（可选）_ 网络名称
- `highlight`：_（可选）_ 是否高亮
- `select`：_（可选）_ 操作是否成功
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_SelectControl.doCrossProbeSelect`。
