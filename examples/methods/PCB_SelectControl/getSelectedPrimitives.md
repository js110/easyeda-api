# PCB_SelectControl.getSelectedPrimitives
...
当你需要在写扩展前先摸清当前上下文，或者需要把 getSelectedPrimitives 的返回值组织成后续逻辑时，这个案例最有参考价值。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_SelectControl_getSelectedPrimitives_' + Date.now();
  ...
  const result = await eda.pcb_SelectControl.getSelectedPrimitives();
  return {
    fixtureName,
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_SelectControl.getSelectedPrimitives`。
