# IPCB_PrimitiveFill.getState_Net
...
return await (async function(eda) {
  const fixtureName = '__codex_example___IPCB_PrimitiveFill_getState_Net_' + Date.now();
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_PrimitiveFill.getState_Net`。
