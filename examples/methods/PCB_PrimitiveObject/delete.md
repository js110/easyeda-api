...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_PrimitiveObject_delete_' + Date.now();
  ...
  const deleted = await eda.pcb_PrimitiveObject.delete([primitiveId]);
...
- `primitiveIds`：二进制内嵌对象的图元 ID 或二进制内嵌对象图元对象
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_PrimitiveObject.delete`。
