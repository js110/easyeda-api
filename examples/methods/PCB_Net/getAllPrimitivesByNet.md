...
当你需要在写扩展前先摸清当前上下文，或者需要把 getAllPrimitivesByNet 的返回值组织成后续逻辑时，这个案例最有参考价值。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_Net_getAllPrimitivesByNet_' + Date.now();
  ...
  const result = await eda.pcb_Net.getAllPrimitivesByNet('GND', []);
  return {
    fixtureName,
...
- `net`：网络名称
- `primitiveTypes`：_（可选）_ 图元类型数组，如若指定图元类型不存在网络属性，返回的数据将恒为空

- 返回值：Promise<Array<[IPCB\_Primitive](../interfaces/IPCB_Primitive.md) >>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_Net.getAllPrimitivesByNet`。
