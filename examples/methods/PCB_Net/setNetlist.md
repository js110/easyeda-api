...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_Net_setNetlist_' + Date.now();
  ...
  const result = await eda.pcb_Net.setNetlist(undefined, 'netlist');
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_Net.setNetlist`。
