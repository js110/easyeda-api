# IPCB_PrimitiveString.setState_Text
...
当你准备把 setState_Text 用在批量改图、重命名或清理脚本里时，先用这个案例确认前置条件和失败信号会更稳。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___IPCB_PrimitiveString_setState_Text_' + Date.now();
  ...
  const before = editable.getState_Text ? editable.getState_Text() : undefined;
  editable.setState_Text('text');
  ...
  const after = afterPrimitive?.getState_Text ? afterPrimitive.getState_Text() : undefined;
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_PrimitiveString.setState_Text`。
