# LIB_Symbol.updateDocumentSource
...
当你需要理解 LIB_Symbol.updateDocumentSource 的调用方式、返回值和使用边界时，这个案例可以作为起点。
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `LIB_Symbol.updateDocumentSource`。
