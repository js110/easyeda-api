...
当你需要在写扩展前先摸清当前上下文，或者需要把 getSchematicInfo 的返回值组织成后续逻辑时，这个案例最有参考价值。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___DMT_Schematic_getSchematicInfo_' + Date.now();
  ...
  const result = await eda.dmt_Schematic.getSchematicInfo(schematicUuid);
  return {
    fixtureName,
...
- `schematicUuid`：原理图 UUID

- 返回值：Promise<[IDMT\_SchematicItem](../interfaces/IDMT_SchematicItem.md) \| undefined>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `DMT_Schematic.getSchematicInfo`。
