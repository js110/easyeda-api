# DMT_Schematic.modifySchematicPageName
...
当你准备把 modifySchematicPageName 用在批量改图、重命名或清理脚本里时，先用这个案例确认前置条件和失败信号会更稳。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___DMT_Schematic_modifySchematicPageName_' + Date.now();
  ...
  const result = await eda.dmt_Schematic.modifySchematicPageName(schematicPageUuid, fixtureName);
...
- `schematicPageUuid`：原理图图页 UUID
- `schematicPageName`：原理图图页名称
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `DMT_Schematic.modifySchematicPageName`。
