# DMT_Schematic.reorderSchematicPages
...
当你准备把 reorderSchematicPages 用在批量改图、重命名或清理脚本里时，先用这个案例确认前置条件和失败信号会更稳。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___DMT_Schematic_reorderSchematicPages_' + Date.now();
  ...
  const result = await eda.dmt_Schematic.reorderSchematicPages(schematicUuid, []);
...
- `schematicUuid`：执行排序的图页所关联的原理图 UUID
- `schematicPageItemsArray`：所有原理图图页属性的数组
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `DMT_Schematic.reorderSchematicPages`。
