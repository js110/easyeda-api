...
当你准备把 modifySchematicPageTitleBlock 用在批量改图、重命名或清理脚本里时，先用这个案例确认前置条件和失败信号会更稳。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___DMT_Schematic_modifySchematicPageTitleBlock_' + Date.now();
  const currentProject = await eda.dmt_Project.getCurrentProjectInfo();
  if (!currentProject) { throw new Error('当前没有打开工程，先补齐环境再调试这个案例。'); }
  const result = await eda.dmt_Schematic.modifySchematicPageTitleBlock(undefined, undefined);
...
- `showTitleBlock`：_（可选）_ 是否显示明细表，不定义将保持当前状态
- `titleBlockData`：_（可选）_ 需要修改的明细项及其修改的值
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `DMT_Schematic.modifySchematicPageTitleBlock`。
