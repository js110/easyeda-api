...
return await (async function(eda) {
  const fixtureName = '__codex_example___SCH_PrimitiveArc_modify_' + Date.now();
  ...
  const createdPrimitive = await eda.sch_PrimitiveArc.create(
    120,
    80,
    160,
    100,
  ...
  try {
    const before = await eda.sch_PrimitiveArc.get(primitiveId);
    const result = await eda.sch_PrimitiveArc.modify(primitiveId, property);
    const after = await eda.sch_PrimitiveArc.get(primitiveId);
    return {
      fixtureName,
      primitiveId,
      property,
    ...
    if (primitiveId) { await eda.sch_PrimitiveArc.delete([primitiveId]); }
  }
})(eda);
```
...
- 返回值：Promise<[ISCH\_PrimitiveArc](./ISCH_PrimitiveArc.md) \| undefined>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `SCH_PrimitiveArc.modify`。
