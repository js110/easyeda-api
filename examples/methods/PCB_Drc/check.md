...
当你需要在写扩展前先摸清当前上下文，或者需要把 check 的返回值组织成后续逻辑时，这个案例最有参考价值。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_Drc_check_' + Date.now();
  ...
  const result = await eda.pcb_Drc.check(true, false, false);
  return {
    fixtureName,
...
- `strict`：是否严格检查，当前 PCB 统一为严格检查模式
- `userInterface`：是否显示 UI（呼出底部 DRC 窗口）
- `includeVerboseError`：是否在返回值中包含详细错误信息，如若为 `true` ，则返回值将始终为数组
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_Drc.check`。
