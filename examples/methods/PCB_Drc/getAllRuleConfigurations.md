# PCB_Drc.getAllRuleConfigurations
...
当你需要在写扩展前先摸清当前上下文，或者需要把 getAllRuleConfigurations 的返回值组织成后续逻辑时，这个案例最有参考价值。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_Drc_getAllRuleConfigurations_' + Date.now();
  ...
  const result = await eda.pcb_Drc.getAllRuleConfigurations(undefined);
  return {
    fixtureName,
...
- `includeSystem`：_（可选）_ 是否获取系统设计规则配置

- 返回值：Promise<Array<{ \[key: string\]: any; }>>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_Drc.getAllRuleConfigurations`。
