...
当你准备把 deleteNetClass 用在批量改图、重命名或清理脚本里时，先用这个案例确认前置条件和失败信号会更稳。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_Drc_deleteNetClass_' + Date.now();
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_Drc.deleteNetClass`。
