# PCB_Drc.modifyPadPairGroupName
...
当你准备把 modifyPadPairGroupName 用在批量改图、重命名或清理脚本里时，先用这个案例确认前置条件和失败信号会更稳。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_Drc_modifyPadPairGroupName_' + Date.now();
  ...
  const result = await eda.pcb_Drc.modifyPadPairGroupName('originalPadPairGroupName', 'padPairGroupName');
...
- `originalPadPairGroupName`：原焊盘对组名称
- `padPairGroupName`：新焊盘对组名称
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_Drc.modifyPadPairGroupName`。
