# PCB_Drc.deleteEqualLengthNetGroup
...
当你准备把 deleteEqualLengthNetGroup 用在批量改图、重命名或清理脚本里时，先用这个案例确认前置条件和失败信号会更稳。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___PCB_Drc_deleteEqualLengthNetGroup_' + Date.now();
  ...
  const result = await eda.pcb_Drc.deleteEqualLengthNetGroup('equalLengthNetGroupName');
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `PCB_Drc.deleteEqualLengthNetGroup`。
