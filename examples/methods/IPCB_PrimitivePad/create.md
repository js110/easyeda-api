...
return await (async function(eda) {
  const fixtureName = '__codex_example___IPCB_PrimitivePad_create_' + Date.now();
  ...
  const result = await primitive.create();
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_PrimitivePad.create`。
