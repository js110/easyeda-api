...
当你需要理解 IPCB_PrimitiveLine.toAsync 的调用方式、返回值和使用边界时，这个案例可以作为起点。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___IPCB_PrimitiveLine_toAsync_' + Date.now();
...
- 返回值：[IPCB\_PrimitiveLine](./IPCB_PrimitiveLine.md)
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_PrimitiveLine.toAsync`。
