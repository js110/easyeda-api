# IPCB_PrimitiveLine.getAdjacentPrimitives
...
当你需要在写扩展前先摸清当前上下文，或者需要把 getAdjacentPrimitives 的返回值组织成后续逻辑时，这个案例最有参考价值。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___IPCB_PrimitiveLine_getAdjacentPrimitives_' + Date.now();
  ...
  const result = await primitive.getAdjacentPrimitives();
...
- 返回值：Promise<Array<[IPCB\_PrimitiveLine](./IPCB_PrimitiveLine.md) \| [IPCB\_PrimitiveVia](./IPCB_PrimitiveVia.md) \| [IPCB\_PrimitiveArc](./IPCB_PrimitiveArc.md) >>
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_PrimitiveLine.getAdjacentPrimitives`。
