# IPCB_PrimitivePoured.addSolderMaskFill
...
当你想把 addSolderMaskFill 放进自动化脚本、批处理或测试脚本时，可以先照着这个案例跑一遍，确认创建后的返回值和清理方式。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___IPCB_PrimitivePoured_addSolderMaskFill_' + Date.now();
  ...
  if (!primitiveId) {
    throw new Error('请先在PCB画布上选中一个与 IPCB_PrimitivePoured 匹配的图元，再运行这个案例。');
  }
  const fetched = await eda.pcb_PrimitivePoured.get([primitiveId]);
  const primitive = Array.isArray(fetched) ? fetched[0] : fetched;
  if (!primitive) { throw new Error('没有取到目标图元对象。'); }
  const result = await primitive.addSolderMaskFill(undefined);
...
- `pourFillId`：IPCB\_PrimitivePouredPourFill \['id'\]
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_PrimitivePoured.addSolderMaskFill`。
