...
当你需要理解 IPCB_PrimitivePolyline.reset 的调用方式、返回值和使用边界时，这个案例可以作为起点。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___IPCB_PrimitivePolyline_reset_' + Date.now();
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `IPCB_PrimitivePolyline.reset`。
