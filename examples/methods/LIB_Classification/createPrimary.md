# LIB_Classification.createPrimary
...
当你想把 createPrimary 放进自动化脚本、批处理或测试脚本时，可以先照着这个案例跑一遍，确认创建后的返回值和清理方式。
...
return await (async function(eda) {
  const fixtureName = '__codex_example___LIB_Classification_createPrimary_' + Date.now();
  ...
  const created = await eda.lib_Classification.createPrimary(libraryUuid, undefined, 'primaryClassificationName');
...
默认验证记录会写到 `examples/reports/latest.json`，该案例的索引键是 `LIB_Classification.createPrimary`。
